...
def _mock_usage(prompt: int, completion: int) -> MagicMock:
    """Мімікрує CrewAI UsageMetrics object."""
    m = MagicMock()
...
class TestRegisterKickoff:
    ...
    def test_basic_kickoff_registration(self, tracker: PipelineCostTracker) -> None:
        usage = _mock_usage(prompt=10_000, completion=5_000)
    ...
    def test_register_with_per_task_breakdown(
        ...
        usage = _mock_usage(15_000, 8_000)
    ...
    def test_register_handles_none_usage_metrics(
        ...
        # None usage_metrics не повинно крашити tracker
        tracker.register_kickoff(
            crew_label="Phase 1: Core",
            usage_metrics=None,
            primary_model="gpt-4o",
    ...
    def test_register_isolated_from_exceptions(
        ...
        # Object що raise при доступі до атрибутів
        class Boom:
            @property
        ...
        # Tracker залишається у валідному стані — kickoff не записаний АБО записаний з 0
        # (поведінка залежить від коли _safe_attr ловить exception)
        # Головне — не упало
...
class TestTotalAggregation:
    ...
    def test_total_sums_all_sources(self, tracker: PipelineCostTracker) -> None:
        ...
        tracker.register_kickoff(
            "Phase 1: Core",
            _mock_usage(10_000, 5_000),
            primary_model="gpt-4o",
        )  # $0.075
        tracker.register_kickoff(
            "Phase 2: Ukrainian",
            _mock_usage(5_000, 3_000),
            primary_model="gpt-4o",
        )  # $0.0125 + $0.03 = $0.0425
        # Embedding
...
class TestOutputSinks:
    ...
    def test_to_dict_returns_dict(self, tracker: PipelineCostTracker) -> None:
        tracker.register_kickoff(
            "Phase 1", _mock_usage(100, 50), primary_model="gpt-4o"
        )
        d = tracker.to_dict()
